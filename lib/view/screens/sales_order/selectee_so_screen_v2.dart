...
class _SelectedSoScreenState extends State<SelectedSoScreen> {
  ...
  void initState() {
    ...
    LineItemCubit.get(context).selectedSOLineItems.clear();
  ...
  Widget _buildSOListTable(List<SalesOrderModel> data) {
  ...
  Widget _buildLineItemsTable(List<SoLineItemModel> data) {
    LineItemSource dataSource = LineItemSource(
      data,
      (SoLineItemModel item) {
        Navigation.push(context, UpdateSoLineItemScreen(lineItem: item));
      },
      onSelectChanged: (SoLineItemModel item, bool isSelected) {
        setState(() {
          if (isSelected) {
            LineItemCubit.get(context).selectedSOLineItems.add(item);
          } else {
            LineItemCubit.get(context).selectedSOLineItems.removeWhere(
                  (element) =>
                      element.listOfSOItem?.iTEMSYSID ==
                      item.listOfSOItem?.iTEMSYSID,
                );
          }
        });
      },
      selectedItems: LineItemCubit.get(context).selectedSOLineItems.toSet(),
    );
    return Container(
      padding: const EdgeInsets.all(8.0),
      color: Colors.white,
...
class LineItemSource extends DataTableSource {
  ...
  final void Function(SoLineItemModel, bool) onSelectChanged;
  final Set<SoLineItemModel> selectedItems;
